  ...
  return (
    <div className="py-8">
      <div className="mb-8">
        ...
        <p className="text-brand-text">Divirta-se enquanto ouve suas músicas favoritas.</p>
      </div>

      {/* Mobile-only message */}
      <div className="md:hidden flex flex-col items-center justify-center py-20 px-8 text-center bg-brand-card rounded-3xl border border-white/5">
        <div className="h-20 w-20 bg-brand-primary/10 rounded-full flex items-center justify-center mb-6">
          <HiPuzzle className="h-10 w-10 text-brand-primary" />
        </div>
      ...
      <div className="hidden md:grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {ALL_GAMES.map((game) => (
          <Link
            key={game.id}
            href={`/games/${game.id}`}
            className="group relative flex flex-col gap-4 overflow-hidden rounded-3xl bg-brand-card p-6 border border-white/5 transition-all hover:scale-[1.03] hover:border-white/20 hover:bg-brand-hover"
          >
            {/* Background Gradient */}
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${game.implemented ? 'bg-brand-primary text-black' : 'bg-white/10 text-white/60'}`}>
                  {game.badge}
                </span>
              </div>
            ...
            <div className="relative z-10 mt-4 flex items-center text-xs font-bold text-white group-hover:text-brand-primary transition-colors">
