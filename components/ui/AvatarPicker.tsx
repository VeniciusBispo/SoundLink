    ...
    <div className="flex flex-col gap-3">
      ...
      {value && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            className="ml-auto rounded-full px-3 py-2 text-xs font-semibold text-red-400 hover:bg-brand-hover"
      ...
      <div className="rounded-xl bg-brand-card/40 p-3 ring-1 ring-white/10">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm font-semibold text-white">Emoji</p>
          <button
            type="button"
            onClick={() => setShowEmojiPicker((v) => !v)}
            className="rounded-full bg-brand-hover px-3 py-2 text-xs font-semibold text-white"
          >
            ...
            <div className="grid max-h-44 grid-cols-8 gap-1 overflow-y-auto pr-1 sm:grid-cols-10">
              {filteredEmojis.map((emoji) => (
                <button
                  key={emoji}
                  type="button"
                  className={`flex min-h-[40px] items-center justify-center rounded-lg text-xl hover:bg-brand-hover active:scale-[0.99] ${value === emoji ? 'ring-2 ring-brand-primary' : ''}`}
      ...
      <div className="flex flex-col gap-2">
        <label className="text-xs text-brand-text">Ou envie uma imagem</label>
        <input
          type="file"
          accept="image/*"
        ...
        {imageUrl && (
          <div className="mt-2 flex flex-col items-center gap-2">
            ...
            <img
              ...
              className="h-32 w-32 rounded-full object-cover border-2 border-brand-primary"
            ...
            <button
              type="button"
              className="min-h-[44px] w-full max-w-xs rounded-xl bg-brand-primary px-4 py-2.5 text-sm font-bold text-black"
              onClick={handleCropAndSave}
            >
              Usar esta imagem
            </button>
            <span className="text-xs text-brand-text">A imagem será cortada e redimensionada para 256x256px</span>
          </div>
        )}
      </div>
    </div>
